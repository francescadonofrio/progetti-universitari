...
$BONUS_XML = __DIR__ . "/xml/bonus.xml";
$crediti_bonus = 0;

if ($username !== "" && is_file($BONUS_XML)) {
  $d = new DOMDocument();
  if (@$d->load($BONUS_XML, LIBXML_NONET)) {
    $xp = new DOMXPath($d);
    $lit = function_exists("xpath_literal")
      ? xpath_literal($username)
      : ("'" . str_replace("'", "&apos;", $username) . "'");
    $n = $xp->query("//utente[@username={$lit}]/crediti")->item(0);
    if ($n) $crediti_bonus = (int)$n->textContent;
  }
}
...
if (isset($conn)) {
  $stmt = mysqli_prepare($conn, "SELECT nome, cognome, email, reputazione, data_registrazione FROM utenti WHERE username=? LIMIT 1");
  mysqli_stmt_bind_param($stmt, "s", $username);
  mysqli_stmt_execute($stmt);
  $row = ($res = mysqli_stmt_get_result($stmt)) ? mysqli_fetch_assoc($res) : null;
  if ($row) $user = $row;
...
<div style="padding:20px; border:1px solid #ccc; border-radius:18px; margin:15px 0;">
  ...
  <form method="post">
    ...
    <p><b>Crediti bonus:</b> <?= htmlspecialchars((string)$crediti_bonus) ?></p>
