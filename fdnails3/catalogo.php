...
require_once __DIR__ . "/promozioni.php";

if (!function_exists("promo_first_child")) {
...
if (!function_exists("promo_child_text")) {
...
<?php foreach ($filtrati as $p): ?>
  <?php
    $promoCount = promo_load_counts($PROMO_XML, (string)$p["id"], $today);
    $prezzoBase = (float)$p["prezzo"];
    $scontoPerc = promo_discount_percent_for($role, (string)$p["id"], $today, $PROMO_XML);
    $prezzoScontato = promo_apply_discount($prezzoBase, $scontoPerc);
  ?>
  <div class="product-card">
    ...
    <p class="game-price">
      <?= number_format($prezzoBase, 2, ",", ".") ?> crediti
      <?php if ($prezzoScontato < $prezzoBase): ?>
        - <?= number_format($prezzoScontato, 2, ",", ".") ?> crediti
      <?php endif; ?>
    </p>
